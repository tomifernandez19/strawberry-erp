            ...
            {selectedOrder ? (
                ...
                <div className="grid mt-md">
                    {orders.length === 0 ? (
                        ...
                        orders.map(order => (
                            <div key={order.id} className="card" style={{ padding: '15px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                    <div>
                                        <h4 style={{ margin: 0 }}>Pedido #{order.nro_pedido}</h4>
                                        <p style={{ fontSize: '0.8rem', opacity: 0.7 }}>{order.cliente_nombre}</p>
                                        <p style={{ fontSize: '0.7rem', opacity: 0.4 }}>{new Date(order.created_at).toLocaleDateString()} {new Date(order.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                                    </div>
                                    <button className="btn-primary" style={{ padding: '8px 16px', fontSize: '0.8rem' }} onClick={() => setSelectedOrder(order)}>Despachar</button>
                                </div>
                                <div style={{ marginTop: '10px', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '10px' }}>
                                    {order.items_raw.map((item, i) => (
                                        <span key={i} style={{ fontSize: '0.75rem', opacity: 0.6, background: 'var(--secondary)', padding: '2px 8px', borderRadius: '4px', marginRight: '5px' }}>
                                            {item.name} ({item.variant_values.join('/')})
                                        </span>
                                    ))}
                                </div>
                            </div>
